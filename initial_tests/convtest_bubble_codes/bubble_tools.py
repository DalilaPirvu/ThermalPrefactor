...
def extract_data(nL, path_sim):
    ...
    return params, reshaped_dat[:,0,:], reshaped_dat[:,1:,:]

def get_realisation(nL, sim, nTimeMAX, path_sim):
...
def check_decay(slice):
    right_phi = len(slice[(slice >  10.)])
    left_phi  = len(slice[(slice < -10.)])
    if right_phi==0 and left_phi==0: return 2
    if right_phi > left_phi: return 0
    else: return 1

def centre_bubble(real):
    nC, nT, nN = np.shape(real)
    tamp = max(0, nT - 2*nN)
    real = real[:,tamp:]

    critslice = np.abs(real[0, 0, :])
...
def get_COM_velocity(simulation, phi_init, crit_thresh, crit_rad, vvv, dx, lamb, tcen, xcen, plots=False):
    ...
    try:
        ...
        if False:
            ...
            ax0.plot(rr*dx, ttwallfit*dx, color='blueviolet', ls=':', linewidth=1, label='rr')
            ax0.plot(ll*dx, ttwallfit*dx, color='orangered', ls=':', linewidth=1, label='ll')
            try:
	            ax0.plot(rrwallfit*dx, ttwallfit*dx, color='blueviolet', ls='-.', linewidth=1, label='r hyp')
	            ax0.plot(llwallfit*dx, ttwallfit*dx, color='orangered', ls='-.', linewidth=1, label='l hyp')
            except:
    ...
    if plots:
     ...
     #   tcen, xcen = find_nucleation_center(simulation, phi_init, crit_thresh, crit_rad)
        ext = np.array([-xcen, nN-xcen, -tcen, nT-tcen])*dx
        im0 = ax.imshow(simulation, interpolation='antialiased', extent=ext, origin='lower', cmap='RdBu', aspect='auto')

        ax.plot(rr*dx, (ttwallfit-tcen)*dx, color='k', ls='--', linewidth=0.5)
        ax.plot(ll*dx, (ttwallfit-tcen)*dx, color='k', ls='--', linewidth=0.5)

        ax.plot(rrwallfit*dx, (ttwallfit-tcen)*dx, color='k', ls='-', linewidth=0.7)
        ax.plot(llwallfit*dx, (ttwallfit-tcen)*dx, color='k', ls='-', linewidth=0.7)
