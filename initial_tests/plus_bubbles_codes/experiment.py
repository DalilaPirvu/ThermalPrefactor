...
def get_general_model(case='free'):
    if case=='free':
        ...
        nTimeMAX = 32768
        tempList = np.array([0.1, 0.2, 0.3, 0.4])
        minSim = 0
        maxSim = 20
...
def get_model(tempList, massq, right_Vmax, V, dV, Vinv, nTimeMAX, minSim, maxSim, tmp, case='free'):
    temp = tempList[tmp]
    m2 = massq(temp)
...
### Paths to files
root_dir      = '/gpfs/dpirvu/prefactor/test_quad_'
...
def get_simulation_energy(real, nLat, lenLat):
