...
! specSpace takes the momentum values of the field
  ...
  subroutine inverse_derivative_1d_wtype(tPair, dk)
    ...
    norm = -iImag / dk / dble(tPair%nx) !the inverse normalisation
    call fftw_execute_dft_r2c(tPair%planf, tPair%realSpace, tPair%specSpace) !direct fft
    do i=2,tPair%nnx !for all modes but the first
       tPair%specSpace(i) = norm*tPair%specSpace(i) / dble(i-1) !normalise/take derivative
    enddo
    tPair%specSpace(1) = 0._dl !the first mode is fixed: PBC
  ...
  ! Takes tPair with field to transform inside %realSpace and fundamental mode
    ...
    norm = dk*iImag / dble(tPair%nx) !same normalisation to multiply each mode

    call fftw_execute_dft_r2c(tPair%planf, tPair%realSpace, tPair%specSpace)
    
    do i=1,tPair%nnx !for all modes
       tPair%specSpace(i) = dble(i-1)*norm*(tPair%specSpace(i)) !take one derivative
    enddo
...
!    tPair%realSpace = (tPair%realSpace)**2._dl !in fact returns square of gradient
  end subroutine gradsquared_1d_wtype
  
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine derivative_1d_wtype(tPair, dk)
  ! A way to compute the derivative of a field f(x) stored in tPair%realSpace
    type(transformPair1D), intent(inout) :: tPair
    real(dl), intent(in) :: dk
    integer :: i
    complex(C_DOUBLE_COMPLEX) :: norm

    norm = dk*iImag / dble(tPair%nx) !normalisation from differentiating once
    call fftw_execute_dft_r2c(tPair%planf, tPair%realSpace, tPair%specSpace) !direct fft
    do i=1,tPair%nnx !for all field modes
       tPair%specSpace(i) = dble(i-1)*norm*(tPair%specSpace(i)) !normalise/take derivative
    enddo
    call fftw_execute_dft_c2r(tPair%planb, tPair%specSpace, tPair%realSpace) !inverse fft
  ...
  ! Same but here there derivation happens twice (i dk)**2 = - dk**2
    ...
    call fftw_execute_dft_r2c(tPair%planf, tPair%realSpace, tPair%specSpace)
    do i=1,tPair%nnx
       tPair%specSpace(i) = dble(i-1)**2 *norm*(tPair%specSpace(i)) !the (i-1) is squared compared to above
    enddo
    call fftw_execute_dft_c2r(tPair%planb, tPair%specSpace, tPair%realSpace)
  ...
  ! Same but here we take the nth derivative where n = order
    type(transformPair1D), intent(inout) :: tPair
    real(dl), intent(in) :: dk
    ...
    norm = (dk*iImag)**order / dble(tPair%nx) ! (i dk)**order
    call fftw_execute_dft_r2c(tPair%planf, tPair%realSpace, tPair%specSpace)
    do i=1,tPair%nnx
       tPair%specSpace(i) = dble(i-1)**order *norm*tPair%specSpace(i) ! factor (i-1) to power of order
    enddo
    call fftw_execute_dft_c2r(tPair%planb, tPair%specSpace, tPair%realSpace)
